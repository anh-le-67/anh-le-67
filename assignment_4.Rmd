...
author: |
  | Anh Le Nguyen Ngoc
  | a.lenguyenngocanh@students.uu.nl
  | https://github.com/anh-le-67
date: June 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW$treat<-with(GrowthSW, ifelse(revolutions>0,'Yes','No'))
...
p-value=0.0687>significance level=0.05.
This means that the evidence is not strong enough to reject null hypothesis: the difference in means between countries that experienced revolutions and countries that did not is equal to 0.
...
'rgdp60'is value of GDP per capita in 1960'. Including this variable into the equation means that GDP of specific countries in 1960 could also affect their growth.
...
From this analysis, education is the main driver of economic drive. When education is added to the model, the change in probability for growth to be explained is the highest (17.5%) and in the last model, 31.8% of the variation in growth is explained by the variation of variables.
...
library(flextable)
list(model1, model2, model3, model4) |>
  modelsummary(stars=T, gof_map = c("nobs", "r.squared"),output="models.docx")
```
